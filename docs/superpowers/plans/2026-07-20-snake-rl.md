...
def test_turn_changes_heading_by_delta():
    ...
    assert abs((w.heading - a0) - np.radians(CFG.turn_deg)) < 1e-6
...
def test_body_length_capped():
    w = fresh(); w.target_length = 999
    assert w.target_length  # sanity; cap enforced on grow (Task 4), here just structural
...
class World:
    ...
    def body_points_uw(self):
        ...
        if len(pts) < 2:
            return np.zeros((0, 2))
        # walk from head (last) backward, emit a point every segment_spacing,
        # skipping the head-adjacent stub (head_radius + spacing + body_radius)
        skip = c.head_radius + c.segment_spacing + c.body_radius
        out, acc, nxt = [], 0.0, skip
        for i in range(len(pts) - 1, 0, -1):
            step = np.linalg.norm(pts[i] - pts[i - 1])
            acc += step
            while acc >= nxt and nxt <= self.target_length:
                out.append(pts[i - 1])
                nxt += c.segment_spacing
            if acc > self.target_length:
                break
...
def test_eat_removes_chicken_and_grows():
    ...
    w.chicken_pos = np.array([w.head + [0.5, 0.0]])   # within eat_radius
    w.chicken_dir = np.array([0.0])
    L0 = w.target_length
    n = w.try_eat()
    assert n == 1
    assert len(w.chicken_pos) == 0
    assert w.target_length == min(CFG.length_cap, L0 + CFG.grow_per_chicken)
    assert w.energy == min(CFG.energy_max, CFG.energy_max + CFG.energy_refill) or w.energy <= CFG.energy_max


def test_growth_capped():
    w = World(CFG, seed=1, size=(60, 60)); w.target_length = CFG.length_cap - 0.1
    w.chicken_pos = np.array([w.head.copy()]); w.chicken_dir = np.array([0.0])
    w.try_eat()
...
def test_chicken_flees_away_when_snake_close():
    ...
    w.chicken_pos = np.array([[30.0 + CFG.r_flee * 0.5, 30.0]]); w.chicken_dir = np.array([0.0])
    before = w.chicken_pos[0].copy()
    w.update_chickens()
    # moved further from snake (positive x direction), by ~v_flee
...
def test_spawn_respects_max():
    ...
    w.chicken_pos = np.zeros((CFG.max_chickens, 2)); w.chicken_dir = np.zeros(CFG.max_chickens)
    for _ in range(1000):
        w.maybe_spawn()
...
def test_nearest_chicken():
    ...
    w.chicken_pos = np.array([[59.0, 1.0], [5.0, 1.0]]); w.chicken_dir = np.zeros(2)
...
```python
    ...
    def maybe_spawn(self):
        ...
        if need or (len(self.chicken_pos) < c.max_chickens and self.rng.random() < 1.0 / c.spawn_period):
            p = self._free_point(c.chicken_radius)
...
def test_step_reports_eat():
    ...
    w.head = np.array([30.0, 30.0]); w.head_uw = w.head.copy()
    w.chicken_pos = np.array([[31.0, 30.0]]); w.chicken_dir = np.array([0.0])  # ahead, will be eaten after move
    out = w.step(steering=1, dash=0)
    assert out["ate"] >= 0   # depending on flee; at minimum no crash
```
...
```python
    def maybe_spawn_forced(self):
        p = self._free_point(self.cfg.chicken_radius)
        self.chicken_pos = np.vstack([self.chicken_pos, p]) if len(self.chicken_pos) else p[None]
        self.chicken_dir = np.append(self.chicken_dir, self.rng.uniform(0, 2 * np.pi))
```
...
def test_smell_stronger_when_closer_and_blocked_by_obstacle():
    w = straight_world()
    w.chicken_pos = np.array([[35.0, 30.0]]); w.chicken_dir = np.array([0.0])
    near = smell(w)[0]
    w.chicken_pos = np.array([[45.0, 30.0]])
    far = smell(w)[0]
    assert near > far
    # place an obstacle between head and a chicken -> intensity drops to ~0
    w.chicken_pos = np.array([[45.0, 30.0]])
...
def test_smell_gradient_points_forward_to_chicken_ahead():
    w = straight_world()
    w.chicken_pos = np.array([[40.0, 30.0]]); w.chicken_dir = np.array([0.0])
    g = smell(w)
...
def test_eat_gives_positive_reward():
    ...
    w = env.world
    w.head = np.array([30.0, 30.0]); w.head_uw = w.head.copy(); w.heading = 0.0
    w.chicken_pos = np.array([[30.6, 30.0]]); w.chicken_dir = np.array([0.0])
    w.obstacle_pos = np.zeros((0, 2)); w.obstacle_r = np.zeros((0,))
    _, r, _, _, info = env.step([1, 0])
    assert info["ate"] == 1 and r > 5


def test_pbrs_closed_loop_nets_near_zero():
    # potential-based shaping telescopes: returning to the same state cancels
    env = SnakeEnv(seed=0); env.reset()
    env.world.chicken_pos = np.array([[45.0, 30.0]]); env.world.chicken_dir = np.array([0.0])
    env.world.head = np.array([30.0, 30.0]); env.world.head_uw = env.world.head.copy()
    env._last_phi = env._phi(); env._last_nearest = env.world.nearest_chicken()[0]
    total, w = 0.0, env.world
    path = [np.array([31.0, 30.0]), np.array([30.0, 30.0])]  # out and back to same phi
    for p in path:
...
class SnakeEnv(gym.Env):
    ...
    def __init__(self, cfg=CFG, seed=None):
        ...
        # obs bounds: ray dist [0,1], one-hots [0,1], smell intensity >=0, grad any, proprio [0,1]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.world = generate_world(self.cfg, seed=s)
        self._last_phi = self._phi()
        self._last_nearest = self.world.nearest_chicken()[0]
        return observe(self.world), {}

    def _shaping(self):
        """PBRS: gamma*phi' - phi, zeroed when the nearest-chicken identity changes."""
        nearest = self.world.nearest_chicken()[0]
        phi = self._phi()
        if nearest != self._last_nearest:
            f = 0.0                                  # target set changed: don't pay the jump
        else:
    ...
    def _on_step(self):
        for info in self.locals["infos"]:
            ...
            if info.get("alive") is False:
                self.deaths += 1
        if self.num_timesteps % self.every < self.locals["env"].num_envs:
...
def _obs_normalizer(norm_path):
    """Load VecNormalize stats to normalize a single env's stacked obs at inference."""
    if not os.path.exists(norm_path):
        return lambda o: o
    dummy = build_vec(1, 0, training=False, norm_path=norm_path)
    stats = dummy
    def norm(o):
        return stats.normalize_obs(o)
    return norm
...
**Spec coverage:** torus nearest-image (Task 2, everywhere) ✓; random torus world + rejection sampling (Task 6) ✓; snake motion/continuous heading (Task 3) ✓; dash+stamina with catch invariant (Tasks 1,3) ✓; chickens wander/flee/spawn/eat (Task 4) ✓; swept collision + death + self-collision skip-by-arc (Task 5) ✓; 9-ray vision + smell(1/(1+r))+LoS + 42-float obs + empty-ray encoding (Task 7) ✓; MultiDiscrete action + PBRS reward + hunger step penalty + truncation (Task 8) ✓; PPO CPU + SubprocVecEnv + VecFrameStack + VecNormalize + checkpoints/stats (Task 9) ✓; pygame render + sensor overlay (Task 10) ✓; watch deterministic + keys + save/load/reset (Tasks 11,12) ✓; feasibility-invariant asserts at init (Task 1,8) ✓; all listed tests present (Tasks 1–8) ✓.
...
**Type consistency:** `World` attributes (`head`, `head_uw`, `heading`, `chicken_pos`, `obstacle_pos/r/kind`, `stamina`, `energy`, `target_length`) used consistently across Tasks 3–11; `observe()->(42,)`, `OBS_DIM=42`, frame_stack=4 → net sees 168, matched in Task 9 test; `SnakeEnv` API matches SB3/gymnasium; `build_vec` wrapper order (`VecNormalize(VecFrameStack(vec))`) matches the `watch.py` accessor note.
