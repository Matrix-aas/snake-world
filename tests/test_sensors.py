...
def test_obs_shape_and_bounds():
    ...
    assert (o[:54].reshape(9, 6)[:, 0] >= 0).all() and (o[:54].reshape(9, 6)[:, 0] <= 1).all()
...
def test_empty_ray_encoding():
    w = straight_world()
    v = sense_vision(w)
    assert (v == np.array([1.0, 0, 0, 0, 0, 0])).all()
...
def test_repro_ready_toggles_on_three_way_gate():
    ...
    s.repro_cooldown = 0
    assert observe(w)[74] == 1.0                       # all three gates pass
    s.repro_cooldown = 5
    assert observe(w)[74] == 0.0                        # cooldown blocks it
    s.repro_cooldown = 0
    s.energy = 0.0
    assert observe(w)[74] == 0.0                        # energy gate blocks it
    s.energy = CFG.repro_energy_frac * CFG.energy_max + 1
    s.target_length = CFG.repro_length_min - 1
    assert observe(w)[74] == 0.0                        # length gate blocks it
